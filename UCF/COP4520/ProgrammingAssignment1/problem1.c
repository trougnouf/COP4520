...
#define MAXTHREADS 4		// Not including the main function. Fastest when MAXTHREADS = CPU threads - 1
...
void printResults(struct timeval * begTime)
{
	...
	// it takes approximately 0.03s - 0.15s to analyze the results on a single thread (-O0 - -O2). This can be optimized using multiple threads.
	if(TESTING == 2)
